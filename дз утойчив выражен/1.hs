...
trans:: String -> String -> Int -> Map.Map String int ->Map.Map String int
   
trans p1 p2 sum bank = Map.fromList [(p1, Map.lookup p1 -  sum ),(p2, Map.lookup p2 + sum )] Здесь не получается вычислить из Maybe - sum, как можно посчитать? 
-}
...
listGender = ['F', 'F', 'F', 'M', 'M', 'M', 'F', 'F', 'F', 'F'] :: [Char]
